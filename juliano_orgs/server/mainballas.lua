...
if Configballas.MaxInService ~= -1 then
  TriggerEvent('esx_service:activateService', 'ballas', Configballas.MaxInService)
end
...
ESX.RegisterServerCallback('esx_ballasjob:getOtherPlayerData', function(source, cb, target)
  ...
  if Configballas.EnableESXIdentity then
    ...
    if Configballas.EnableLicenses then
